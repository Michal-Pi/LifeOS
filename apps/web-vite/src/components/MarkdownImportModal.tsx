/**
 * Markdown Import Modal Component
 *
 * Allows users to import Projects, Chapters, and Tasks from Markdown files or pasted text.
 */
...
import { importMarkdownProject } from '@/lib/markdownImportService'
import type { ImportResult } from '@/lib/taskImportService'
import { MarkdownPreviewPanel } from './MarkdownPreviewPanel'
import { TaskMarkdownPreviewPanel } from './TaskMarkdownPreviewPanel'
import { MarkdownFormatDocModal } from './MarkdownFormatDocModal'
...
export function MarkdownImportModal({
  ...
  const { user } = useAuth()
  const [importMode, setImportMode] = useState<'project' | 'tasks'>('project')
  const [activeTab, setActiveTab] = useState<'upload' | 'paste'>('upload')
  const [isDragging, setIsDragging] = useState(false)
  const [showDocModal, setShowDocModal] = useState(false)
  const fileInputRef = useRef<HTMLInputElement>(null)
  const lastTaskImportResultRef = useRef<ImportResult | null>(null)

  // Todo operations hook for projects, chapters, and createTask
  const { projects, chapters, createTask, loadData } = useTodoOperations({
    userId: user?.uid || '',
  })
  ...
  const {
    ...
    setMarkdown: setProjectMarkdown,
    parseAndValidate: parseAndValidateProject,
    reset: resetProject,
  ...
  const {
    ...
    setMarkdown: setTaskMarkdown,
    parseAndValidate: parseAndValidateTasks,
    reset: resetTasks,
  ...
  const state = importMode === 'project' ? projectState : taskState
  const markdown = importMode === 'project' ? projectMarkdown : taskMarkdown
  const errors = importMode === 'project' ? projectErrors : taskErrors
  const setMarkdown = importMode === 'project' ? setProjectMarkdown : setTaskMarkdown
  const parseAndValidate =
    importMode === 'project' ? parseAndValidateProject : parseAndValidateTasks
  const reset = importMode === 'project' ? resetProject : resetTasks
  const setState = importMode === 'project' ? setProjectState : setTaskState

  const handleCreateProject = useCallback(async () => {
    if (!parsedProject || !user?.uid) return

    setProjectState('creating')
    try {
      const result = await importMarkdownProject(user.uid, parsedProject)
      toast.success('Project imported successfully', {
        description: `Created ${result.totalCreated} items (1 project, ${result.chapterIds.length} chapters, ${result.taskIds.length} tasks)`,
      })
      ...
      setProjectState('preview') // Go back to preview so user can retry
    }
  }, [parsedProject, user, setProjectState, onImportComplete])
  ...
  }, [taskImportResult, taskState, onImportComplete])
  ...
  const handleImportTasks = useCallback(async () => {
    ...
    try {
      ...
      setTaskState('preview') // Go back to preview so user can retry
    }
  }, [parsedTasks, user, projects, chapters, createTask, createTasks, setTaskState])
  ...
  const handleClose = useCallback(() => {
    ...
    resetTasks()
    onClose()
  }, [resetProject, resetTasks, onClose])
  ...
  return (
    <div className="modal-overlay" onClick={handleClose}>
      <div className="modal-content markdown-import-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="import-mode-selector">
            <button
              type="button"
              className={`mode-button ${importMode === 'project' ? 'active' : ''}`}
            ...
            <button
              type="button"
              className={`mode-button ${importMode === 'tasks' ? 'active' : ''}`}
          ...
          {activeTab === 'paste' && (
            <div className="paste-area">
              <label htmlFor="markdown-textarea" className="textarea-label">
          ...
          {/* Preview State */}
          {state === 'preview' && importMode === 'project' && parsedProject && (
            <div className="preview-container">
              <p className="success-message">
                ✓ Markdown parsed successfully! Review the structure below.
              </p>
              <MarkdownPreviewPanel project={parsedProject} />
            </div>
          )}
          ...
          {state === 'creating' && (
            <div className="loading-state">
              <p>
                {importMode === 'project'
                  ? 'Creating project, chapters, and tasks...'
                  : 'Creating tasks...'}
              </p>
            </div>
          )}
          ...
          {state === 'success' && (
            <div className="success-state">
              <p className="success-message">✓ Import completed successfully!</p>
            </div>
          )}
        </div>
            ...
            <>
              ...
              {importMode === 'project' ? (
                <button
                  ...
                  disabled={!parsedProject || !user?.uid}
                >
                  Create Project
                </button>
              ) : (
      ...
      {/* Documentation Modal */}
      <MarkdownFormatDocModal isOpen={showDocModal} onClose={() => setShowDocModal(false)} />
    </div>
  )
}
