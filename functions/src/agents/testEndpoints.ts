...
export const testAgentConfig = onCall({}, async (request) => {
  ...
  const { agentId, workflowId } = request.data as {
    agentId?: string
    workflowId?: string
  }
  if (!agentId || !workflowId) {
    throw new HttpsError('invalid-argument', 'agentId and workflowId are required.')
  }

  const db = getFirestore()
  const agentDoc = await db
    .doc(`users/${request.auth.uid}/workflows/${workflowId}/agents/${agentId}`)
    .get()
