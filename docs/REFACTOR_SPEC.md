...
```typescript
const { addedClaimIds } = await mapClaimsToKG(
  contextClaims,
  contextSources,
  sharedKg,
  sessionId,
...
const claimLines = scoredClaims.sort((a, b) => b.score - a.score).slice(0, 10)
```
...
function extractActionObject(decision: string): { verb: string; object: string } | null {
  ...
  const match = decision.match(
    /^(increase|decrease|expand|contract|...)\s+(?:the\s+|a\s+|an\s+)?(.+?)(?:\s+(?:by|to|from|in)\b|[,.]|$)/i
  )
...
if (!apiKeys || Object.keys(apiKeys).length === 0)
  throw new Error('Dialectical graph requires API keys')
```
...
if (budgetPause) {
  log.warn(`Budget exceeded before ${nodeName}`, {
    runId: state.runId,
...
function mapToSteep(raw: string): TrendObject['steepCategory'] {
  ...
  if (lower.includes('environ') || lower.includes('ecolog') || lower.includes('climate'))
    return 'environmental'
  if (lower.includes('politic') || lower.includes('regulat') || lower.includes('govern'))
    return 'political'
  if (lower.includes('social') || lower.includes('societ') || lower.includes('demograph'))
    return 'social'
...
const inputHash = hashObject({
  ...
  crossImpact: state.crossImpactMatrix,
})
...
- functions/src/agents/**tests**/ — Most test files live here
- functions/src/agents/oracle/**tests**/ — Oracle-specific tests
- functions/src/agents/langgraph/**tests**/ — LangGraph-specific tests
- functions/src/agents/deepResearch/**tests**/ — Deep Research tests
...
File: functions/src/agents/oracle/**tests**/oracleResume.test.ts
...
File: functions/src/agents/oracle/**tests**/oracleEvidence.test.ts
...
File: functions/src/agents/**tests**/contradictionMapping.test.ts
...
File: functions/src/agents/langgraph/**tests**/deepResearchKGEvolution.test.ts
...
File: functions/src/agents/**tests**/kgEnrichment.test.ts (add to existing)
...
File: functions/src/agents/oracle/**tests**/oracleBudget.test.ts
...
File: functions/src/agents/oracle/**tests**/oracleGateEscalation.test.ts
...
File: functions/src/agents/langgraph/**tests**/dialecticalConfigClone.test.ts
...
File: functions/src/agents/langgraph/**tests**/deepResearchSeeding.test.ts
...
File: functions/src/agents/oracle/**tests**/oracleTokens.test.ts
...
const scenarioDriverSets = state.scenarioPortfolio.map(
  (s) => new Set(s.keyDrivers?.map((d) => d.toLowerCase()) ?? [])
)
...
- participatingClaims contains KG claim IDs (format: claim*<timestamp>*<random>)
...
const hasValidEvidence =
  state.context?.researchEvidence && state.context.researchEvidence.claims?.length > 0
