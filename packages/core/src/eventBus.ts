...
export interface EventBus<EventMap extends Record<string, unknown> = Record<string, unknown>> {
  publish<EventKey extends keyof EventMap>(event: EventKey, payload: EventMap[EventKey]): Promise<void>;
...
export class SimpleEventBus<EventMap extends Record<string, unknown> = Record<string, unknown>>
  implements EventBus<EventMap>
{
  ...
  subscribe<EventKey extends keyof EventMap>(event: EventKey, handler: EventHandler<EventMap[EventKey]>) {
